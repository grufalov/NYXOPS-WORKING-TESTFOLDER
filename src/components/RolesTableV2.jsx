...
function hydrateCols(storageKey) {
...
/* =========================
   Component
========================= */
export default function RolesTableV2({ 
  rows = [], 
  loading = false, 
  storageKey = "rolesTableV2_cols_v1",
  onViewRow = () => {},
  onEditRow = () => {},
  onDeleteRow = () => {},
  onLoadMore = () => {},
  ...
  const SORT_STORAGE_KEY = `${storageKey}_sort_v1`;
  ...
  }, [sortDescriptor, SORT_STORAGE_KEY]);

  // column UI state (with persistence)
  const [cols, setCols] = useState(() => hydrateCols(storageKey));
  useEffect(() => {
  ...
  const filtered = useMemo(() => {
    // Filtering is now handled by parent component - just return all rows
    return rows;
  }, [rows]);
  ...
  const empty = (
    <div className="py-12 text-center">
      <div className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-[color:rgba(0,0,0,0.04)] text-[13px] text-[var(--muted-text)]">
        <span>{loading ? "Loading..." : "No rows to display."}</span>
      </div>
    </div>
  );
      ...
      case "actions":      return (
        <div className={"flex items-center " + justifyClass + " gap-1.5"}>
          <button
            ...
            aria-label="View" onClick={(e)=>{e.stopPropagation(); onViewRow(item);}} title="View">
          ...
          <button
            ...
            aria-label="Delete" onClick={(e)=>{e.stopPropagation(); onDeleteRow(item);}} title="Delete">
            <svg width="16" height="16" viewBox="0 0 20 20" fill="none">
  ...
  return (
    <>
      {/* Card wrapper */}
      <div className="rounded-2xl border shadow-md bg-[var(--card-bg)] z-10">
        ...
        <div className="relative overflow-x-auto rounded-2xl">
          <Table
            ...
            classNames={{
              base: "w-full",
              table: "table-fixed min-w-[1100px] border-collapse bg-[var(--card-bg)]",
              thead: "sticky top-0 z-30 bg-[var(--card-bg)]",
              th: [
              ...
              tr: [
                "cursor-pointer transition-colors bg-[var(--card-bg)]",
              ...
              td: [
                "text-[13px] py-3 px-4 first:rounded-l-xl last:rounded-r-xl whitespace-nowrap align-middle bg-[var(--card-bg)]",
                "border-r border-black/10 first:border-l last:border-r-0",
              ].join(" "),
            }}
                    ...
                    return (
                      <TableCell
                        ...
                        className={[
                          ...
                          col.key === "actions"
                            ? "sticky right-0 z-40 bg-[var(--card-bg)] before:absolute before:inset-y-0 before:-left-px before:w-px before:bg-black/10"
                            : "",
                          isRisk
                            ? "max-w-[320px] align-middle pr-6 overflow-hidden whitespace-normal break-words"
                            : isTrunc
                              ? "max-w-[160px] pr-6 overflow-hidden truncate align-middle"
        ...
        <div className="flex items-center justify-between px-3 py-2 border-t border-black/10 rounded-b-2xl bg-[var(--card-bg)]">
      ...
      <Modal isOpen={isEditOpen} onOpenChange={setIsEditOpen} size="lg" backdrop="blur" placement="center">
        <ModalContent>
          ...
          <ModalBody className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <Select
              label="Practice"
              selectedKeys={new Set([form.practice ?? ""])}
              onSelectionChange={(keys) => setForm((f) => ({...f, practice: Array.from(keys)[0]}))}
            >
              {["Consulting", "Engineering", "Design"].map((p) => <SelectItem key={p}>{p}</SelectItem>)}
            </Select>
            <Select
              label="Client"
              selectedKeys={new Set([form.client ?? ""])}
              onSelectionChange={(keys) => setForm((f) => ({...f, client: Array.from(keys)[0]}))}
            >
              {["Acme Corp", "Beta LLC", "Gamma Inc", "Delta Co"].map((c) => <SelectItem key={c}>{c}</SelectItem>)}
            </Select>
            <Select
              label="Status"
            ...
            >
              {["Open", "On Hold"].map((s) => <SelectItem key={s}>{s}</SelectItem>)}
            </Select>
